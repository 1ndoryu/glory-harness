...
pub(crate) struct EstadoGit {
    ...
    /// `None` si no se pudo resolver). [139A-2]
...
/// (detached), `HEAD@<7>`; si tampoco hay HEAD (sin commits), `None`. [139A-2]
async fn rama_en(raiz: &Path) -> Option<String> {
    let actual = ejecutar_git(raiz, &["branch", "--show-current"]).await.ok()?;
...
#[cfg(test)]
mod tests {
    use super::{descubrir_repos, parsear_status};
