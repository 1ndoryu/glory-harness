...
- [x] **129A-8 — Resumen de cambios al finalizar el turno (estilo Synara)**
      ...
      `PanelCambios.revelar(ruta)` abre carpeta + diff vivo. Sin cambios = sin
      bloque. 17/17 asserts CDP + captura Synara.
...
- [ ] **139A-8 — Remediación de la auditoría integral (EN EJECUCIÓN, plan v5
      reconciliado 13-09)** (pedido 13-09: plan para resolver la auditoría
      `Agente/documentacion/auditoria-integral-2026-09-13.md` v2 verificada):
      F0 re-resolución de citas → F1 seguridad ejecución (K1 jaula, K13
      secreto en log, K3 loopback, K9 cookie firmada, K12 rate-limit std) →
      F2 SQLite async + historial acotado (R1/R2/R3, p95 87,81→3,83 ms) →
      F3n ALTA restantes (K2 entorno, K4 hooks exec, K5 jaula cwd, K6 SSRF
      hooks, K7 MCP stdio) → F4 puertos y god-objects (S1–S5) → F5n
      unificación path + SQLite (S7/S8/K8-TOCTOU/R4) → F6n streaming/DOM
      (R5/R6/R8, R7 front condicional por 139A-9) → F7n pin glory-sentinel +
      cierre (este repo solo sube el pin). v5 invalida F3–F6 v1–v4
      (etiquetas no-auditadas, trabajo inventado, 5 ALTA fuera).
      ...
      puntual, p95 turno 87,81 ms → 3,83 ms; F3n cerrada 13-09 (gate
      PASS coverage/sccache/sentinel/rust, 12 tests nuevos k4/k6/k7/entorno):
      `entorno.rs` (`env_clear` + allowlist) en jaula/hooks/MCP/ejecutor/git,
      allowlist binarios hooks + `Hook::comando_interno`, `cd` con args
      denegado, SSRF hooks (denegar por defecto + `egreso_extra`), allowlist
      MCP + `GLORY_MCP_ALLOW`; en curso F4.
