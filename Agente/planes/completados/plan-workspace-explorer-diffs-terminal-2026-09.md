...
- `desktop/ui/src/componentes/panelDerecho.ts`: tabs del panel derecho y launcher/estado vacío (incluye launcher estilo Synara, 089A-4).
- `desktop/ui/src/componentes/barraSuperior.ts`: barra global estilo Synara (089A-3, 46px, botonera ventana).
- `desktop/ui/src/componentes/menu.ts`: menú contextual compartido.
- `desktop/ui/src/tauri/real.ts`: adaptación de comandos y eventos Tauri, incluido `onCambioArchivo` (trunca diff a 600 caracteres).
- `desktop/ui/src/estilos/tabs.css`, `layout.css`, `launcher.css`, `barraSuperior.css` y estilos de los paneles: sistema visual monocromo ya existente.
...
**Nota:** el motor `content_search` (tgrep-core v1.0.5, 089A-8) existe como tool del agente en `core/src/herramientas/archivo/content_search.rs`. Es indexado, con smart-case y timeout 180s, registrado como `CAT_LECTURA`, y compilado dentro del binario. **No es un comando IPC Tauri** — solo lo usa el runtime del agente. No reemplaza la búsqueda de filesystem por nombre desde la UI, que sigue pendiente de implementar como comando Tauri.
...
### 3.4 Estado post-implementaciones 089A-7/8

Las herramientas de archivo se reorganizaron (089A-7, HECHO):

- `core/src/herramientas/tools_archivo.rs` se movió a `core/src/herramientas/archivo/tools_archivo.rs`;
- `ToolContentSearch` se extrajo a `core/src/herramientas/archivo/content_search.rs`;
- `archivo/mod.rs` actúa como fachada, reexportada desde `herramientas/mod.rs`.

La búsqueda por contenido `content_search` (089A-8, HECHO) es un motor indexado in-process con `tgrep-core` v1.0.5:

- usa `regex` + `sandbox.leer` (secretos excluidos);
- smart-case, timeout 180 s, índice persistente por workspace;
- registrada como tool `CAT_LECTURA` del agente (en `subagente.rs` y `politica/regla.rs`);
- **no es un comando Tauri IPC**; la UI no puede consultarlo directamente.

El código del launcher del panel derecho (089A-4) ya ofrece Navegador, Visor y Chat lateral desde una pantalla de inicio — reutilizable para Files/Terminal futuros. La tarea sigue marcada como pendiente/en curso en `roadmap.md` hasta completar la verificación visual del usuario.

La infraestructura visual básica para el explorador ya existe: tabs, launcher, barra superior, scrollbar personalizada. Lo que falta no es cosmético, son los comandos Tauri de filesystem, los adaptadores de árbol y los contratos de cambio.
...
- `desktop/ui/src/tauri/real.ts` expone eventos de tools y `onCambioArchivo`, pero el contrato actual entrega `diffHtml`, no un patch estructurado. Sigue truncando `ev.diff?.slice(0, 600)` — la observación del plan sobre el límite de diffs sigue vigente.
...
- `content_search` (089A-8) es un motor de búsqueda por contenido indexado, compilado dentro del binario del núcleo (`tgrep-core` v1.0.5), registrado como tool del agente. **No es un comando Tauri IPC ni un endpoint de la UI:** el frontend no puede consultarlo. No sustituye ni la búsqueda por nombre que propone la Fase 1 ni el explorador de archivos.
- `tools_archivo.rs` se movió a `archivo/tools_archivo.rs` (089A-7). Las tool IDs, contratos y límites (`MAX_LECTURA_BYTES`, `FILE_SEARCH_MAX_RESULTADOS`, etc.) no cambiaron.
- Ya existe un launcher del panel derecho (089A-4) que puede extenderse para añadir Files y Terminal como opciones adicionales.
...
## 8. Fases de implementación (estado 2026-09-08)

> **Decisión ponytail:** La Fase 5 (watcher nativo con `notify` crate) se
> descarta para el MVP porque `recargar()` + botón manual + `onCambioArchivo`
> cubren la detección de cambios externos sin añadir una dependencia pesada.
> La Fase 6 (terminal PTY con ConPTY) se difiere: requiere emulador DOM
> (xterm.js o custom), proceso concurrente por tab, resize y cleanup, y no
> hay necesidad reportada. El plan `089A-9` se cierra con Fases 0–4
> completadas; watcher y terminal se reabren como tarea separada si el uso
> real lo justifica.
...
- ¿Se necesita búsqueda de contenido en la UI? `content_search` (089A-8) ya ofrece búsqueda indexada para el agente, pero expuesta como tool del núcleo, no como endpoint Tauri IPC. Si la Fase 2 necesitara búsqueda de contenido desde la UI, habría que decidir entre (a) exponer el motor existente como comando Tauri o (b) mantener búsqueda de nombres únicamente.
...
6. No conectar `content_search` directamente a la UI en este bloque: primero medir si Files necesita búsqueda de contenido y, si la necesita, definir un comando Tauri separado con el mismo límite de workspace y errores estructurados.
